...
def filtered_transient_obs(geomodel_shapefile, seasonal_df):
    ...
    output_path = '../data/data_waterlevels/obs/05_Transient_groundwater_obs.xlsx'
...
def extract_steady_state():
    ...
    output_path = '../data/data_waterlevels/obs/06_Steady_state_groundwater_obs.xlsx'
    steady_state_df.to_excel(output_path, index=False)

def plot_seasonal_flows(geomodel_shapefile):
    WL_df = pd.read_excel('../data/data_waterlevels/obs/05_Transient_groundwater_obs.xlsx')
    model_boundary = gpd.read_file(geomodel_shapefile)
