...
class Flowmodel:
    ...
    def __init__(self, scenario, project, inputs, geomodel, observations = None, *args, **kwargs):     
    ...
    def write_flowmodel(self, transient = False, 
        ...
        dv2d = Disv2Disu(self.geomodel.mesh.vertices, 
                         self.geomodel.mesh.cell2d, 
                         self.geomodel.top[0], 
                         self.geomodel.botm, 
                         staggered=self.staggered, 
                         disv_idomain = self.geomodel.idomain,)
    ...
    def plot_plan(self, spatial, array, layer, extent = None, 
        ...
        if self.mesh.plangrid == 'car': self.mesh.sg.plot(ax = ax, color = 'black', lw = 0.2)
        if self.mesh.plangrid == 'tri': self.mesh.tri.plot(ax = ax, edgecolor='black', lw = 0.2)
        if self.mesh.plangrid == 'vor': self.mesh.vor.plot(ax = ax, edgecolor='black', lw = 0.2)
        ax.plot([extent[0], extent[1]], [extent[2], extent[3]], color = 'black', lw = 1)
        plt.tight_layout() 
        plt.savefig('../figures/plan_%s.png' % array)

    def plot_transect(self, geomodel, array, title = None, grid = True,
        ...
        x0 = kwargs.get('x0', self.geomodel.mesh.x0)
        y0 = kwargs.get('y0', self.geomodel.mesh.y0)
        x1 = kwargs.get('x1', self.geomodel.mesh.x1)
        y1 = kwargs.get('y1', self.geomodel.mesh.y1)
